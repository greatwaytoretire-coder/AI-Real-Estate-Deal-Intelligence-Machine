...
class EnhancedBuyerProfile:
    ...
    buyer_id: str = field(
        default_factory=lambda: f"bp_{uuid4()}"
    )
    ...
    data_freshness_timestamp: str = field(
        default_factory=lambda: datetime.now(UTC).isoformat()
    )

    confidence_score: float = 0.0

    criteria: BuyerInvestmentCriteria = field(
        default_factory=BuyerInvestmentCriteria
    )

    activity: BuyerActivity = field(
        default_factory=BuyerActivity
    )

    contact_info: BuyerContactInfo = field(
        default_factory=BuyerContactInfo
    )
...
class BuyerReliabilityScorer:
    ...
    def score(
        ...
        reasoning = [
            "Base score for profile existence."
        ]

        if profile.verification_status == "VERIFIED":
            score += 30.0
            reasoning.append(
                "+30 points for VERIFIED status."
            )

        if profile.proof_of_funds_verified:
            score += 20.0
            reasoning.append(
                "+20 points for verified proof of funds."
            )
        ...
        if profile.activity.deals_closed_last_12m > 0:
            ...
            activity_bonus = min(
                20.0,
                profile.activity.deals_closed_last_12m * 5,
            )

            score += activity_bonus

            reasoning.append(
                f"+{activity_bonus} points for recent transaction history."
            )

        if profile.data_source in [
            "user_provided",
            "crm_import",
        ]:
            score += 10.0
            ...
            reasoning.append(
                "+10 points for high-quality data source."
            )
...
class BuyerDealMatcher:
    ...
    def match(
        self,
        deal: DealContext,
        buyers: List[EnhancedBuyerProfile],
        ...
        for buyer in buyers:
            ...
            if deal.zip_code in buyer.criteria.geographic_preferences:
                ...
                match_reasons.append(
                    "Matches preferred ZIP code."
                )

            else:

                mismatch_reasons.append(
                    "Deal is outside preferred ZIP codes."
                )


            if (
                buyer.criteria.price_range_min
                <= deal.purchase_price
                <= buyer.criteria.price_range_max
            ):

                score += 25

                match_reasons.append(
                    "Price is within buyer's range."
                )

            else:

                mismatch_reasons.append(
                    "Price is outside buyer's range."
                )
            ...
            if deal.investment_strategy in buyer.criteria.investment_strategies:
                ...
                match_reasons.append(
                    "Matches investment strategy."
                )


            reliability = self.reliability_scorer.score(
                buyer
            )

            score += reliability.score * 0.25

            match_reasons.append(
                f"Reliability score of {reliability.score:.0f} applied."
            )
